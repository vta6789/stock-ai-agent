...
*** LƯU Ý QUAN TRỌNG (bài học từ bug đã gặp 2 lần): hàm chay_toan_bo_watchlist()
BẮT BUỘC phải gọi luu_toan_bo_phien_len_dau(ket_qua_de_luu) ở cuối - nếu thiếu
dòng này, pipeline vẫn chạy "thành công" và log "HOÀN TẤT PHIÊN PHÂN TÍCH" bình
thường, NHƯNG không hề ghi gì vào Excel cả, và KHÔNG có bất kỳ log lỗi nào cảnh
báo việc này (vì code đơn giản là không chạy tới đoạn lưu). Đây là lỗi ÂM THẦM
nguy hiểm nhất trong toàn bộ dự án - nếu sửa file này trong tương lai, luôn xác
nhận lại dòng gọi hàm lưu này còn tồn tại trước khi coi là xong việc.
"""
...
# ---- Định dạng Excel ----
COT_TONG_QUAN = ["Mã", "Trạng thái", "Độ tin cậy", "Giá vào", "Cắt lỗ", "Chốt lời", "Mức độ rủi ro", "Nhận định & lời khuyên", "Thời gian phân tích"]
DO_RONG_TONG_QUAN = [5, 6, 5, 10, 10, 10, 14, 60, 20]
        ...
        try:
            ...
            for i_dong, (quyet_dinh, ma) in enumerate(ket_qua_hop_le):
                ...
                gia_tri = [
                    ...
                    quyet_dinh.risk_level, quyet_dinh.nhan_dinh_tong_quan, thoi_diem,
                ]
                for idx, v in enumerate(gia_tri, start=1):
                    ws_tong_quan.cell(row=dong_moi, column=idx, value=v)

                ws_tong_quan.cell(row=dong_moi, column=8).alignment = Alignment(wrap_text=True, vertical="top")
...
def chay_toan_bo_watchlist(danh_sach_ma: list = None) -> list:
    ...
    # ****** QUAN TRỌNG - ĐỪNG XÓA DÒNG NÀY ******
    # Đây chính là dòng đã bị thiếu 2 lần trước đó, khiến pipeline "chạy xong"
    # nhưng KHÔNG ghi gì vào Excel cả mà không hề có log lỗi cảnh báo.
    if ket_qua_de_luu:
